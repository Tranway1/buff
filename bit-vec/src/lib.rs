...
impl BitVec<u32> {
    ...
    ///
    /// by Chunwei
    /// init a bit-vec with given vector and required bit size.
    pub fn from_vec(vec: &mut Vec<B>, nbits:usize) -> Self {
        let nblocks = blocks_for_bits::<B>(nbits);
        while vec.len()< nblocks{
            vec.push(B::zero());
        }
        let mut bit_vec = BitVec {
            storage: vec.to_vec(),
    ...
    ///
    /// by Chunwei
    /// set storage with given vector and required bit size.
    #[inline]
    pub fn set_storage(&mut self, vec: &Vec<B>){
        let len = vec.len();
        self.storage[0..len].copy_from_slice(vec);
    }


    ///
    /// by Chunwei
    /// get storage entry by given index
    #[inline]
    pub fn get_entry(&self, i:usize) -> B{
        return *self.storage.get(i).unwrap();
    }
    ...
    ///
    /// by Chunwei
    /// convert bit vector into binary format.
    pub fn to_binary(&self) -> Vec<u8> {
        let mut buffer: Vec<u8> = Vec::new();
        buffer.extend((self.nbits as u64).to_le_bytes().to_vec());
        for &entry in self.storage.iter(){
            buffer.extend(entry.to_le_bytes().to_vec());
        }
        buffer.to_vec()
    }
    ...
    ///
    /// by Chunwei
    /// init a bit-vec with given binary input.
    pub fn from_binary(data: &[u8]) -> Self {
        let mut store: Vec<u32> = Vec::new();
        let mut cnt = 8;
        let nbits = u64::from_le_bytes(data[0..cnt].try_into().unwrap()) as usize;
        let offset = B::bytes();
        let len = data.len()-offset;
        while cnt<=len{
            store.push(u32::from_le_bytes(data[cnt..cnt+offset].try_into().unwrap()));
            cnt += offset;
        }

        let mut bit_vec = BitVec {
            storage: store.to_vec(),
            nbits,
        };
        bit_vec.fix_last_block();
        bit_vec
...
impl<B: BitBlock> BitVec<B> {
    ...
    pub fn remove_range(&mut self, s: usize, e:usize) {
        self.ensure_invariant();
        assert!(e < self.nbits, "index out of bounds: {:?} >= {:?}", e, self.nbits);
        let sw = s / B::bits();
        let sb = s % B::bits();
        let ew = e / B::bits();
        let eb = e % B::bits();
        if sw==ew{
            let mask = !(((!B::zero())>>sb)<<(sb+B::bits()-eb)>>(B::bits()-eb));
            self.storage[ew] = (self.storage[ew] & mask);
        }
        else {
            if sb==0{
                self.storage[sw] = B::zero();
            }
            else {
                self.storage[sw] = (self.storage[sw] << (B::bits()-sb)) >> (B::bits()-sb);
            }
            self.storage[sw] = (self.storage[sw] << (B::bits()-sb)) >> (B::bits()-sb);
            if sw+1<ew{
                for w in &mut self.storage[sw+1..ew] { *w = B::zero(); }
            }
            self.storage[ew] = (self.storage[ew] >> eb) << eb;
        }
...
mod tests {
    ...
    fn test_bit_vec_remove() {
        let mask = !(((!0u32)>>7)<<(7+32-16)>>(32-16));
        println!("{:b}",mask);
        let mut bit_vec = BitVec::from_bytes(&[
            0b01001001, 0b10010010, 0b10111101,0b01001001,
            0b10010010, 0b10111101,0b01001001, 0b10010010,
            0b10111101,0b01001001, 0b10010010, 0b10111101,
            0b01001001,0b10111101,0b01001001, 0b10010010,
            0b10010010, 0b10111101,0b01001001, 0b10010010,
            0b10111101,0b01001001, 0b10010010, 0b10111101]);
        bit_vec.remove_range(7,76);
        assert_eq!(bit_vec, BitVec::from_bytes(&[
            0b01001000, 0b00000000, 0b00000000,0b00000000,
            0b00000000, 0b00000000,0b00000000, 0b00000000,
            0b00000000,0b00001001, 0b10010010, 0b10111101,
            0b01001001,0b10111101,0b01001001, 0b10010010,
            0b10010010, 0b10111101,0b01001001, 0b10010010,
            0b10111101,0b01001001, 0b10010010, 0b10111101]));
        ...
        let mut bit_vec = BitVec::from_bytes(&[
            ...
            0b10010010, 0b10111101,0b01001001, 0b10010010,
            0b10111101,0b01001001, 0b10010010, 0b10111101]);
        bit_vec.remove_range(7,8);
        assert_eq!(bit_vec, BitVec::from_bytes(&[
            0b01001000, 0b10010010, 0b10111101,0b01001001,
            0b10010010, 0b10111101,0b01001001, 0b10010010,
            0b10111101,0b01001001, 0b10010010, 0b10111101,
            0b01001001,0b10111101,0b01001001, 0b10010010,
            0b10010010, 0b10111101,0b01001001, 0b10010010,

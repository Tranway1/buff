...
impl RoaringBitmap {
    ...
    /// Adds a bit vector directly to the roaring bitmap. Returns `true` if it was done.
    /// by Chunwei
    /// # Examples
    ///
    pub fn insert_direct_u32(&mut self, value: u32, bit_word:u32) -> bool {
        let (key, index) = util::split(value);
        let container = match self.containers.binary_search_by_key(&key, |c| c.key) {
            Ok(loc) => &mut self.containers[loc],
            Err(loc) => {
                self.containers.insert(loc, Container::new_bitmap(key));
                &mut self.containers[loc]
            }
        };
        container.insert_bitmap(index, bit_word)
    }
    ...
    /// ensure right format for each container
    pub fn optimize(&mut self){
        self.containers.iter_mut().map(|container| container.ensure_correct_store());
    }

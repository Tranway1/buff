...
use crate::compress::buff_slice::{flip, BYTE_WORD, set_pred_word, avx_iszero, ceil};
...
pub fn run_scaled_slice_encoding_decoding(test_file:&str, scl:usize, pred: f64) {
    ...
    // comp.scaled_slice_range_filter_nosimd(comp_sum,pred);
    let duration5 = start5.elapsed();
    println!("Time elapsed in scaled slice range no simd function() is: {:?}", duration5);

    let start6 = Instant::now();
    // comp.scaled_slice_equal_filter_nosimd(comp_max,pred);
    let duration6 = start6.elapsed();
    println!("Time elapsed in scaled slice eqaul no simd function() is: {:?}", duration6);

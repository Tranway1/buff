...
//            println!("{:#066b} => {:#066b}", preu, bdu);
    ...
    pub(crate) fn offset_encode<'a,T>(&self, seg: &mut Segment<T>) -> Vec<u8>
        where T: Serialize + Clone+ Copy+Into<f64> + Deserialize<'a>{
    ...
    pub(crate) fn byte_decode(&self, bytes: Vec<u8>) -> Vec<f64>{
        ...
        if (remain>=8){
            ...
            if remain == 0 {
                for (int_comp,dec_comp) in int_vec.iter().zip(chunk.iter()){
                    ...
                    expected_datapoints.push(cur_intf + cur_intf.signum() *(((*dec_comp)) as f64) / dec_scl);
                }
        ...
        while (remain>=8){
            ...
            if remain == 0 {
                ...
                for (int_comp,dec_comp,cur_chunk) in izip!(&int_vec,&dec_vec,chunk){
                    ...
                    // }
                    // j += 1;
                    expected_datapoints.push( cur_intf + cur_intf.signum() *(((*dec_comp)|((*cur_chunk) as u32)) as f64) / dec_scl);
                }
            }
            else{
        ...
        if (remain>0){
            ...
            for (int_comp,dec_comp) in int_vec.iter().zip(dec_vec.iter()){
                ...
                // }
                // j += 1;
                expected_datapoints.push( cur_intf + cur_intf.signum() *(((*dec_comp)|(bitpack.read_bits( remain as usize).unwrap() as u32)) as f64) / dec_scl);
            }
        }

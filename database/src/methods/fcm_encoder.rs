...
impl<'a,T> FCMCompressor<'a,T>
    where T:Eq+Hash+Clone+Copy+Num+Into<u32>{

    pub fn new(input: Vec<T>, level: usize, def: bool) -> FCMCompressor<'a,T> {
        FCMCompressor{
            len: input.len(),
            input: input.iter().map(|&x|x.into()).collect(),
    ...
    fn  get_prev_list(&self) -> Option<Vec<u32>> {
        ...
        if start >= 0 {
            let ret: Vec<u32> = self.input[start..self.count].to_vec();
            return Some(ret);
        }
        None
    }
    ...
    fn predict(&self) -> Option<u32>  {
        if self.count >= self.level {
            let vec_u32 = self.get_prev_list().unwrap();
            Some(*self.array_list_map.get(&vec_u32).unwrap())
//            Some(*self.array_list_map.get(&serialize(self.get_prev_list().unwrap()).unwrap()).unwrap())
    ...
    pub fn differential_compress(&mut self) -> Vec<u8>  {
        ...
        let ratio= vec.len() as f32 / (vec_len as f32* num_bits as f32);
        print!("{}",ratio);
        vec
    }

    pub fn delta_compress(&mut self) -> Vec<u8>  {
        let mut mydata = &self.input.clone();
        let mut num_bit = 0;
        let mut bits = 0;
        ...
        if self.delta{
            ...
            num_bit = b;
            delta_vec = vec;
            mydata = delta_vec.as_ref();
        }
        else {
            num_bit = num_bits(mydata);
        }
